...
assert.match(html, /<main id="main">\s*<section id="concepts"/, 'Start with the five concepts');
...
for (const concept of ['agent', 'skill', 'connector', 'subagent', 'context']) {
  assert(ids.includes(`concept-${concept}`), `Missing concept ${concept}`);
  assert(html.includes(`value="${concept}" aria-controls="concept-${concept}"`), `Missing selector for ${concept}`);
  assert(fs.readFileSync(`${__dirname}/style.css`, 'utf8').includes(`input[value="${concept}"]:checked) .concept-panel:not(#concept-${concept})`), `Missing panel switching for ${concept}`);
}
...
console.log('PASS: page anchors, five concepts, travel example, and native controls.');
